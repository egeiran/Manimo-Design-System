...
- **Next ITGK batch — continuing after the 2026-06-29 ch.8 grunn-vs-dyp-kopi scene.**
  ...
  `innstikksortering-kortene`, `arv-og-overstyring`, and tonight's
  `grunn-vs-dyp-kopi` for ch.8 — a depth companion to
  `liste-referanser`. Three matrix cards (a amber, b teal, c teal)
  render the nested list `[[1, 2], [3, 4]]` as a 2x2 grid; a dashed
  "samme indre" bracket links a and b after `b = a.copy()`. Beat 3
  carries the first genuine data-model motion: `a[0][0] = 99`
  flips BOTH a's and b's (0,0) cell simultaneously and the bracket
  pulses rose, payoff `[[99, 2], [3, 4]] [[99, 2], [3, 4]]`. Beat
  4 carries the second: `c = copy.deepcopy(a)` materialises c
  independent (no bracket), b fades, then `a[0][0] = 100` flips
  only a's cell while c stays at 99, payoff
  `[[100, 2], [3, 4]] [[99, 2], [3, 4]]`. 70 s, 4 explicit
