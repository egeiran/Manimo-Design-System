...
- **ripple-counter chainSetup — Q labels appear twice on each stage (inside box right edge + outside box as wire label).** In `motion/ade/ripple-counter.jsx` `ChainSetupBeat`, `TffBox` renders the Q label at `(right - 14, cy + 6)` inside the box, and the wire-labelling block at delay 3.0..3.6 also renders `qNames[i]` at `(cx + boxW/2 + 6, cy - 14)`. Both read individually but together they're a duplicate. Suggested fix: drop the inner Q label from `TffBox` (let the outer one own the signal name) so the box body shows only J=1/K=1/CLK, and the wire reads "Q_i" once just outside the box. New scene added 2026-05-20.
...
- **ADE chapter map** (Supabase `public.chapters` for `subject_id='ade'`, 10 chapters as of 2026-05-13): ch.1 Kretsteori, ch.2 Energi og effekt, ch.3 Superposisjon og Thévenin, ch.4 Dioder, ch.5 Digital elektronikk, ch.6 Transistorer, ch.7 Minne og register, ch.8 Reaktive elementer, ch.9 Operasjonsforsterker, ch.10 Digital design. Current scene coverage (after the 2026-05-20 run): ch.1 (KVL, voltage-divider, KCL, ohms-law, **current-divider** — added tonight), ch.2 (capacitor-energy, inductor-energy, max-power-transfer, joule-heating), ch.3 (Thévenin, Norton, superposition, source-transformations), ch.4 (half-wave-rectifier, full-wave-bridge-rectifier, zener-clipper, diode-iv-curve), ch.5 (NAND universality, two's-complement, hexadecimal-counting, binary-addition, full-adder), ch.6 (mosfet-switch, bjt-load-line, cmos-inverter, bjt-current-mirror), ch.7 (D flip-flop, shift-register, SR latch, jk-flip-flop, **ripple-counter** — added tonight), ch.8 (phasor-rotation, low-pass-bode, rc-charging, rl-transient, rlc-resonance, **high-pass-bode** — added tonight), ch.9 (inverting op-amp, non-inverting op-amp, summing-amplifier, integrator-op-amp), ch.10 (K-map, FSM, multiplexer-4-to-1, 2-to-4 decoder). Most under-served chapters now: ch.10 (4 scenes — candidates: ripple-carry adder block diagram, 8-to-3 priority encoder, demultiplexer), ch.1 (5 scenes — mesh / nodal analysis, dependent sources still open). Other strong follow-ups: T flip-flop standalone (ch.7), AC impedance triangle (ch.8), photodiode / Schottky (ch.4), CMOS NAND/NOR (ch.6), ASCII / character codes (ch.5).
