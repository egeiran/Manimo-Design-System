...
- **BJT load-line Beat 3 (loadLine) — italic "I_C" axis label is partially eclipsed by the "I_C vs V_CE" mono section title in landscape.** In `motion/ade/bjt-load-line.jsx` `LoadLineBeat`, the title sits at `G.gy − 22` and the italic `I_C` axis label sits at `G.gy − 4`, both stacked above the y-axis top. The title carries the meaning so the italic label is largely redundant, but if you want both visible the I_C label needs to move further (e.g. y = G.gy + 16 with `textAnchor="end"` and `x = G.gx − 16`, swapping order with the V_CC/R_C tick label) or just delete the italic axis label entirely.
...
- **ADE chapter map** (Supabase `public.chapters` for `subject_id='ade'`, 10 chapters as of 2026-05-13): ch.1 Kretsteori, ch.2 Energi og effekt, ch.3 Superposisjon og Thévenin, ch.4 Dioder, ch.5 Digital elektronikk, ch.6 Transistorer, ch.7 Minne og register, ch.8 Reaktive elementer, ch.9 Operasjonsforsterker, ch.10 Digital design. Current scene coverage: ch.1 (KVL, voltage-divider), ch.2 (capacitor-energy), ch.3 (Thévenin), ch.4 (half-wave-rectifier), ch.5 (NAND universality, two's-complement), ch.6 (mosfet-switch, bjt-load-line), ch.7 (D flip-flop), ch.8 (phasor-rotation, low-pass-bode), ch.9 (inverting op-amp), ch.10 (K-map, FSM). All 10 chapters now have at least one scene. Strongest under-served candidates for upcoming runs: full adder / ripple carry (ch.10), full-wave bridge rectifier (ch.4), Kirchhoff's current law (ch.1), MOSFET inverter cascade / CMOS (ch.6 deepening), RC charging in the ADE namespace (ch.8 reactive).
