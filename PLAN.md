...
- **Next algdat batch — thirteenth scene after tonight's 2026-06-29 `rekursjonstre-arbeidsrader` (ch.0 Verktøykassa, the chapter's first scene).**
  Coverage so far: ch.0 `rekursjonstre-arbeidsrader` (tonight — for
  n = 8, a four-level recursion tree expands DOWN one level at a
  time; each level k carries 2^k nodes with per-node cost n/2^k, and
  a row-total badge lights up on the right after each level lands
  asserting that row sums to n; the summing beat then highlights all
  four totals in place and lands the payoff pill n · log₂ n;
  takeaway is T(n) = O(n log n); pedagogical hook is "the
  recursion tree IS the spreadsheet"), ch.1 `asymptotisk-vekst`,
  ch.3 `flettesortering`, ch.4 `tellesortering-bokser`, ch.5
  `binaerhaug-sift`, ch.6 `dp-fylling-fib`, ch.7
  `aktivitetsvalg-graadig`, ch.8 `bfs-bolgen`, ch.9
  `kruskal-trygg-kant`, ch.10 `dijkstra-slapp-av`, ch.11
  `floyd-warshall-mellomstopp`, ch.12 `ford-fulkerson-augment`.
  Strongest remaining motion follow-ups in priority order: a ch.2
  reduction scene `reduksjon-3sat-til-klikk` (vertex per literal,
  clause-clusters, picking a satisfying assignment maps to picking
  one vertex per cluster forming a clique — the only ch.2 topic
  with genuine motion), a second ch.9 depth scene
  `prim-cut-frontier` (Prim grows the MST from one vertex outwards,
  the cut frontier expanding edge by edge, complementary to
  Kruskal's sort-and-sweep), a ch.6 depth companion like
  `dp-rod-cutting` (1-D table fills with each cut option overlaid),
  `quicksort-partisjon` for ch.3 (Lomuto partition with a pivot,
  walker, and two-region sweep), or a ch.12 depth companion
  `min-cut-from-residual` (BFS layer from S in the residual graph
  reveals the min-cut edges after Ford-Fulkerson converges). Skip
  ch.13 / ch.14 (NP-classes) — they animate poorly. All algdat
  ...
  milestones as fractions of sprite duration, dual-aspect
  layoutGeom with usePortrait()).

- **Next ITGK batch — continuing after the 2026-06-30 ch.8 listekomprimering scene.**
  Twenty ITGK scenes are now in the manifest (founding five plus
  ...
  `innstikksortering-kortene`, `arv-og-overstyring`,
  `grunn-vs-dyp-kopi`, and tonight's `listekomprimering` for ch.8 —
  the third ch.8 depth scene, demonstrating list comprehensions as
  a compact for-loop. Code panel on the left, two-row data stage on
  the right (source row `tall=[1..5]` above, transform glyph `(·)²`
  middle, result row `kvadrater` below). Beat 2 runs the verbose
  variant (empty list + for + append) and the result row grows one
  amber cell per iteration. Beat 3 runs the same job as
  `[x*x for x in tall]` — each x flies UP through the glyph and
  DOWN into the result row, same outcome as beat 2. Beat 4 adds the
  filter `if x % 2 == 0`: odd values get rejected with a rose chip
  falling off-cell + an AVVIST tag, even values pass through and
  land in a shorter result row [4, 16]. 77 s, 4 explicit beats +
  SceneChrome intro. Strongest next topics, in priority
  ...
  like vektorisering-vs-pythonlokke (ch.12 — same dot product two
  ways, side by side with a wall-clock ticker so the vectorised op
  finishes while the Python loop is still walking);
  break/continue-løkkekontroll (ch.5 — a value flowing through a
  loop body either fires print or short-circuits back to the
  header / breaks out entirely, the misconception is that break
  exits ALL enclosing loops); valgsortering-finn-minste (ch.11 —
...
- **[HUMAN] binaerhaug-sift dualView portrait — array cells 5–7 clipped off the right edge.** In `motion/algdat/binaerhaug-sift.jsx` `DualViewBeat`, the portrait `layoutGeom` places the 8-cell array row with `arrayLeftX=60`, `arrayPitch=70`, `arrayCell=60` — but the dualView portrait snapshot (t≈17.14 s) shows only cells 0–4 (values 55, 20, 50, 15, 18) on-canvas; cells 5, 6, 7 (values 40, 45 and the empty/dashed slot for incoming 90) sit off the right edge. The dual-correspondence story (tree-node ↔ array-cell at the same index) is broken in portrait because half the array is invisible. Not a single-coordinate fix — needs the whole array row to shrink (`arrayPitch` 70 → ~58 and/or `arrayCell` 60 → ~46) so 8 × pitch + cell fits within svgW=720, plus a re-derive of the correspondence-line endpoints that consume those constants. Landscape is unaffected. Reviewer flagged on 2026-06-28.

- **[HUMAN] grunn-vs-dyp-kopi `SharedBracket` label "samme indre" sits over card-a's right edge.** In `motion/itgk/grunn-vs-dyp-kopi.jsx` `SharedBracket`, the label is centred at `cx = G.sharedX` (landscape 690, portrait 320) at fontSize 13 — but the gap between card a (right edge at aX+cardW = 680 landscape / 300 portrait) and card b (left edge bX = 760 landscape / 420 portrait) is 80 px landscape / 120 px portrait, and the rendered label is ~85 px wide. The text clips into card a's right grid edge in both aspects of beats 2 and 3. The dashed bracket itself reads fine; only the centred text label crowds card a. Fix options: (a) drop label fontSize from 13 → 11 so it fits cleanly in the 80 px landscape gap, (b) move the label OUT of the bracket band — render it above the bracket band as a separate centred caption between the two card titles, or (c) widen the inter-card gap in both `layoutGeom` branches (e.g. landscape bX 760 → 800; portrait bX 420 → 460). Not BROKEN, just tight. Reviewer flagged on 2026-06-29.
...
- **algdat chapter map** (Supabase `public.chapters` for
  ...
  problemer. Coverage after the 2026-06-29 nightly (Norwegian, not yet
  published): ch.0 (`rekursjonstre-arbeidsrader` — added tonight,
  the chapter's first scene: recursion tree for T(n)=2T(n/2)+n
  unrolls DOWN four levels for n=8, each row sums to n, payoff
  n·log₂ n lands in beat 4, takeaway T(n)=O(n log n)), ch.1
  (`asymptotisk-vekst`), ch.3 (`flettesortering`), ch.4
  (`tellesortering-bokser`), ch.5 (`binaerhaug-sift`), ch.6
  (`dp-fylling-fib`), ch.7 (`aktivitetsvalg-graadig`), ch.8
  (`bfs-bolgen`), ch.9 (`kruskal-trygg-kant`), ch.10
  (`dijkstra-slapp-av`), ch.11 (`floyd-warshall-mellomstopp`), ch.12
  (`ford-fulkerson-augment` — added 2026-06-28, the chapter's first
  scene: four nodes S/A/B/T diamond and five directed weighted
  edges with `flyt / kapasitet` fractions; three augmenting paths
  in sequence — S-A-T sends 4 and saturates A-T (amber), S-B-T
  sends 6 and saturates S-B (amber), then the punchline S-A-B-T
  sneaks past the locked A-T using the still-open A-B with
  bottleneck 2 to deliver the last two units; teal flow chip
  travels along each path during HL phase, flow numerators tick
  from old to new during COMMIT, saturated edges turn amber on
  HOLD; total counter ticks 0 → 4 → 10 → 12; payoff maks flyt =
  12). Strongest motion follow-ups (priority order): a ch.9 depth
  scene `prim-cut-frontier` (Prim grows the MST from one vertex
  outwards, cut frontier expanding edge by edge, complementary to
  Kruskal's sort-and-sweep), a ch.6 depth companion like
  `dp-rod-cutting` (1-D table fills with each cut option overlaid,
  optimal-substructure made visible), a ch.3 depth companion
  `quicksort-partisjon` (Lomuto partition with a pivot, walker,
  and two-region sweep — complements flettesortering's split-and-
  merge), or a ch.12 depth companion `min-cut-from-residual` (BFS
  layer from S in the residual graph reveals min-cut edges after
  Ford-Fulkerson converges, max-flow-min-cut visible). Skip
  ch.13/ch.14 (theory, animates poorly) for now. Norwegian audio
...
- **ITGK chapter map** (Supabase `public.chapters` for `subject_id='itgk'`,
  ...
  (TDT4109). Coverage after the 2026-06-23 ch.2 nightly: ch.2
  (`referanse-vs-verdi` — added tonight, the chapter's first scene:
  two VarBoxes a=5 and b=5 sit teal-side-by-side after `b = a`, then
  during `b = b + 1` b's value lifts out, gets struck out, and a
  fresh amber 6 chip drops in from above — a is untouched, utskrift
  `5 6`. The list contrast follows: row [1, 2, 3] with both name
  tags pointing in, rose 4 grows in place, utskrift `[1, 2, 3, 4]`
  twice. Side-by-side compare-cards close with the rule
  `int · str · tuple — kopierer · list · dict · set — deler`, 69 s,
  5 beats), ch.3 (`f-strenger-hullene-fylles` — added
  2026-06-20, two teal value chips glide from VarBoxes navn and alder
  into the highlighted slots of the f-string template, slots flip
  amber→teal as each chip lands, closing beat shows {pris * antall}
  evaluating in three steps to 100), ch.4
  (`betingelser-jernbanevekslere` — added 2026-06-18, value capsule
  descends a rail through three if/elif/else switches, only the first
  true branch lights), ch.5 (`while-lokke-trace`, `for-lokke-range` —
  added tonight 2026-06-21: a depth-not-coverage companion to the
  while scene, `range(1, 6)` builds a row of five teal cells with a
  struck-through ghost `6` showing the half-open stop, then in
  ...
  2026-06-15), ch.8 (`liste-referanser`, `grunn-vs-dyp-kopi` —
  added tonight 2026-06-29, a depth companion to liste-referanser:
  three matrix cards (a amber, b teal, c teal) render
  `[[1, 2], [3, 4]]` as a 2x2 grid; a dashed "samme indre"
  bracket links a and b after `b = a.copy()`. Beat 3 flips both
  cards' (0,0) cell to 99 simultaneously while the bracket pulses
  rose; beat 4 fades b, materialises an independent c via
  `copy.deepcopy(a)`, then mutates `a[0][0] = 100` to show only a
  changes while c stays at 99 — 70 s, 4 beats + SceneChrome
  intro), ch.9
  ...
  `binaersok-halvering`, `innstikksortering-kortene` — added tonight
  2026-06-26, the chapter's first sorting scene: six values
  [5, 2, 4, 6, 1, 3], the rose lifted card hovers above the row
  while sorted-region amber border grows left-to-right. Beat 3 is
  one full insertion of the 2; beat 4 chains 4, 6, 1, 3 over eleven
  CSS-transitioned substeps and ends in a teal "ferdig" badge
  reading 1, 2, 3, 4, 5, 6 — 58 s, 5 beats), ch.12 (`numpy-broadcasting` — added
