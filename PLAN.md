...
- **Inductor-energy `rampingCurrent` portrait — coil sits low in canvas, leaving a tall empty band above the title.** In `motion/ade/inductor-energy.jsx` `RampingCurrentBeat`, the portrait branch parks `coilCy=360` and `srcY=200`, which works geometrically but leaves the top ~100 px of the 720-px canvas unused while the formula `v = L · di/dt` is squeezed at `formulaY=540`. Bumping `coilCy` up by ~40 px (and the wires/source with it) would re-centre the diagram and let the formula breathe.

- **Joule-heating `chargesThroughR` landscape — V bracket label hugs the resistor edge.** In `motion/ade/joule-heating.jsx` `ChargesThroughResistorBeat`, the voltage bracket spans `zigStartX..zigEndX` (full resistor width) with the `V` text at `(zigStart+zigEnd)/2`. At the 380-px resistor in landscape that span is generous, but the `V` ends up directly above the centre zig peak — visually fine but it could move ~6 px up for a touch more breath. Cosmetic only.
...
- **ADE chapter map** (Supabase `public.chapters` for `subject_id='ade'`, 10 chapters as of 2026-05-13): ch.1 Kretsteori, ch.2 Energi og effekt, ch.3 Superposisjon og Thévenin, ch.4 Dioder, ch.5 Digital elektronikk, ch.6 Transistorer, ch.7 Minne og register, ch.8 Reaktive elementer, ch.9 Operasjonsforsterker, ch.10 Digital design. Current scene coverage (after the 2026-05-16 run): ch.1 (KVL, voltage-divider, KCL), ch.2 (capacitor-energy, inductor-energy, max-power-transfer, joule-heating — three added 2026-05-16), ch.3 (Thévenin, Norton), ch.4 (half-wave-rectifier, full-wave-bridge-rectifier), ch.5 (NAND universality, two's-complement), ch.6 (mosfet-switch, bjt-load-line), ch.7 (D flip-flop, shift-register), ch.8 (phasor-rotation, low-pass-bode), ch.9 (inverting op-amp, non-inverting op-amp), ch.10 (K-map, FSM, multiplexer-4-to-1). Most under-served chapters now: ch.3 (2 scenes — candidate: superposition), ch.7 (2 scenes — candidates: SR latch, JK flip-flop, ring counter), ch.9 (2 scenes — candidates: summing amp, integrator/differentiator, comparator). Other strong follow-ups: full adder / ripple carry (ch.10 — pending PR #20 resolution), 2-to-4 decoder (ch.10 — pair with MUX), RC charging in the ADE namespace (ch.8 reactive), CMOS inverter (ch.6 deepening).
