...
- **Mat2B chapter map** (Supabase `public.chapters` for `subject_id='mat2b'`, 6 chapters): ch.1 Vektorrom, ch.2 Lineærtransformasjoner, ch.3 Indreproduktrom, ch.4 Differensialligninger, ch.5 Funksjoner og derivasjon, ch.6 Ekstremalpunkter. Coverage after 2026-05-21 PM2 nightly: ch.1 (span-and-dependence, dimension-intuition, subspace-test, linear-combination-recipe — 4), ch.2 (rank-nullity-visual, change-of-basis-matrix, basis-change-grid, matrix-as-function, linear-transformation-grid, determinant-as-area — 6), ch.3 (projection-onto-line, best-approximation, gram-schmidt-2d-then-3d, **inner-product-geometry**, **orthogonal-complement**, **least-squares-normal-equations** — added tonight — 6), ch.4 (5), ch.5 (4), ch.6 (3). Most under-served now: ch.6 Ekstremalpunkter (3 — Lagrange multipliers is the biggest missing topic). Other strong follow-ups: basis-uniqueness (ch.1 — every vector has unique coordinates in a basis), composition-of-matrices (ch.2), Cauchy-Schwarz-inequality (ch.3), chain-rule-on-path (ch.5), polar-double-integral (ch.5 / new ch.7?), divergence-as-flux (vector calc — may need new chapter row).
