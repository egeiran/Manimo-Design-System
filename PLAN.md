...
- **Inverting op-amp `KclResultBeat` portrait — i_in and i_f labels stack tightly under the resistor rows.** In `motion/ade/inverting-op-amp.jsx` `KclResultBeat`, the i_in / i_f text labels sit 28-30 px below their respective resistors, which collides with the result formulas above the caption in portrait (vbH=460 → captionY=440). Cosmetic. Suggested fix: shift the i-arrow labels to sit just above the resistors (rInY - 22, rfY - 24) in portrait only.
...
- **[HUMAN] Re-run `npm run publish norton-equivalent non-inverting-op-amp shift-register`** once Supabase env vars are available on the nightly sandbox. Tonight's reviewer skipped publish because keys were missing. Reviewer flagged on 2026-05-15.
...
- **ADE chapter map** (Supabase `public.chapters` for `subject_id='ade'`, 10 chapters as of 2026-05-13): ch.1 Kretsteori, ch.2 Energi og effekt, ch.3 Superposisjon og Thévenin, ch.4 Dioder, ch.5 Digital elektronikk, ch.6 Transistorer, ch.7 Minne og register, ch.8 Reaktive elementer, ch.9 Operasjonsforsterker, ch.10 Digital design. Current scene coverage (after the 2026-05-17 run): ch.1 (KVL, voltage-divider, KCL), ch.2 (capacitor-energy, inductor-energy, max-power-transfer, joule-heating), ch.3 (Thévenin, Norton, **superposition** — added 2026-05-17), ch.4 (half-wave-rectifier, full-wave-bridge-rectifier), ch.5 (NAND universality, two's-complement), ch.6 (mosfet-switch, bjt-load-line), ch.7 (D flip-flop, shift-register, **SR latch** — added 2026-05-17), ch.8 (phasor-rotation, low-pass-bode), ch.9 (inverting op-amp, non-inverting op-amp, **summing-amplifier** — added 2026-05-17), ch.10 (K-map, FSM, multiplexer-4-to-1). Most under-served chapters now: ch.1 (3 scenes — candidates: Ohm's law primer, series/parallel), ch.4 (2 scenes — candidates: Zener / clipper, photodiode), ch.5 (2 scenes — candidates: hexadecimal / number-systems primer), ch.6 (2 scenes — candidates: CMOS inverter, BJT current-mirror), ch.8 (2 scenes — candidates: RC charging, RLC resonance). Other strong follow-ups: full adder / ripple carry (ch.10 — pending PR #20 resolution), 2-to-4 decoder (ch.10 — pair with MUX), JK flip-flop (ch.7 — deepens memory), integrator op-amp (ch.9 — deepens op-amps).
