...
- **[HUMAN] Re-run `npm run publish partial-derivatives-as-slices surface-and-contour-duality implicit-curve-slope`** once Supabase env vars are wired in. Three new ch.5 Funksjoner og derivasjon scenes added on the 2026-05-22 late-night Mat2B nightly run; manifest + spec carry `subject_id='mat2b'` + `chapter_number=5` but `public.scenes` rows have not been written yet.
...
- **Mat2B chapter map** (Supabase `public.chapters` for `subject_id='mat2b'`, 6 chapters): ch.1 Vektorrom, ch.2 Lineærtransformasjoner, ch.3 Indreproduktrom, ch.4 Differensialligninger, ch.5 Funksjoner og derivasjon, ch.6 Ekstremalpunkter. Coverage after 2026-05-22 late-night Mat2B nightly: ch.1 (span-and-dependence, dimension-intuition, subspace-test, linear-combination-recipe, basis-uniqueness, null-space-as-line, polynomial-vectors — 7), ch.2 (rank-nullity-visual, change-of-basis-matrix, basis-change-grid, matrix-as-function, linear-transformation-grid, determinant-as-area — 6), ch.3 (projection-onto-line, best-approximation, gram-schmidt-2d-then-3d, inner-product-geometry, orthogonal-complement, least-squares-normal-equations — 6), ch.4 (euler-step, euler-vs-rk4, second-order-to-system, diagonalisation-eigenaxes, phase-portrait-2x2, separable-variables-circles, newtons-law-of-cooling, characteristic-roots-regimes — 8), ch.5 (gradient-and-level-curves, directional-derivative, tangent-plane-linearisation, multivariable-limit-paths, chain-rule-on-path, **partial-derivatives-as-slices**, **surface-and-contour-duality**, **implicit-curve-slope** — added tonight — 8), ch.6 (global-extrema-triangle, hesse-eigenvalues, hessian-test, lagrange-multipliers, critical-points-gradient-field — 5). Most under-served now: ch.6 (5). Strongest follow-ups: composition-of-matrices (ch.2 — chained linear maps), Cauchy-Schwarz-inequality (ch.3), exact-ODE-recognition or integrating-factor-linear-ODE (ch.4 — would fill the last classification slot), quadratic-taylor-approximation (ch.6 — links linearisation → Hessian test), KKT-or-boundary-extrema (ch.6 — constrained on a closed region), saddle-point-via-Hessian-eigenvalues (ch.6 — pair with hessian-test), polar-double-integral (would need a new ch.7 for multiple integration), divergence-as-flux (vector calc — would need a new ch.8).
