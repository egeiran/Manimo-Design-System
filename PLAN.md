...
- **Inverting op-amp Beat 3 (virtualShort) — "V+ = 0" callout collides with the bottom "+" pin label.** In `motion/ade/inverting-op-amp.jsx` `VirtualShortBeat`, the `V+ = 0` text sits at `(G.gndX + 28, G.gndY - 28)` which in landscape places it right next to the bottom op-amp input's `+` glyph (drawn by `OpAmpShape`). Readable but visually noisy. Either move the callout further down/right of the ground symbol, or anchor it to the V+ wire segment (between op-amp pin and ground) rather than near the pin itself. Single-coordinate tweak.

- **Inverting op-amp Beat 5 (sweepDemo) — landscape V_out trace slightly clips the bottom of the plot area on peak negative.** In `motion/ade/inverting-op-amp.jsx` `SweepBeat` landscape, `amp = G.gh * 0.18` and `ampOut = 2·amp` means the negative peak reaches `midY + ampOut ≈ 230 + 122 = 352`, well inside `[gy, gy+gh]=[60,400]`, so mathematically it fits — but visually the rose trace skims the cursor dashed line and the V_out label squeezes against the bottom axis. Either drop `ampOut` to `1.7·amp` or raise the V_out label position by ~12 px. Single-file polish.
...
- **ADE chapter map** (Supabase `public.chapters` for `subject_id='ade'`, 10 chapters as of 2026-05-13): ch.1 Kretsteori, ch.2 Energi og effekt, ch.3 Superposisjon og Thévenin, ch.4 Dioder, ch.5 Digital elektronikk, ch.6 Transistorer, ch.7 Minne og register, ch.8 Reaktive elementer, ch.9 Operasjonsforsterker, ch.10 Digital design. Current scene coverage: ch.1 (KVL, voltage-divider), ch.2 (capacitor-energy), ch.3 (Thévenin), ch.4 (half-wave-rectifier), ch.5 (NAND universality), ch.7 (D flip-flop), ch.8 (phasor-rotation, low-pass-bode), ch.9 (inverting op-amp), ch.10 (K-map, FSM). Open chapter with no scene yet: **6 (Transistorer)**. Strong candidates for the next nightly run: BJT load-line analysis (ch.6), MOSFET as a switch (ch.6), full adder / ripple carry (ch.10), binary number system / two's complement (ch.5). Pick from ch.6 first to fill the last open chapter.
