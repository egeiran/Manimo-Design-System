...
- **[HUMAN] Re-run `npm run publish quadratic-taylor-approximation partial-derivative-as-slice clairaut-mixed-partials`** once Supabase env vars are wired in. Three new Mat2B scenes added tonight (2026-05-23 nightly): `quadratic-taylor-approximation` (ch.6 — links tangent-plane-linearisation → hessian-test via a saddle-slice order sweep with live residual bar), `partial-derivative-as-slice` (ch.5 — geometric meaning of ∂f/∂x with a scrubbing slicing plane y = y₀ and live slope readout), `clairaut-mixed-partials` (ch.5 — symmetry of mixed partials via two-path computation + shrinking corner-sum rectangle around (0.9, 0.5)). Manifest + spec carry `subject_id='mat2b'` with chapters 6, 5, 5 respectively but `public.scenes` rows have not been written yet.
...
- **Mat2B chapter map** (Supabase `public.chapters` for `subject_id='mat2b'`, 6 chapters): ch.1 Vektorrom, ch.2 Lineærtransformasjoner, ch.3 Indreproduktrom, ch.4 Differensialligninger, ch.5 Funksjoner og derivasjon, ch.6 Ekstremalpunkter. Coverage after 2026-05-23 nightly: ch.1 (span-and-dependence, dimension-intuition, subspace-test, linear-combination-recipe, basis-uniqueness, null-space-as-line, polynomial-vectors — 7), ch.2 (rank-nullity-visual, change-of-basis-matrix, basis-change-grid, matrix-as-function, linear-transformation-grid, determinant-as-area — 6), ch.3 (projection-onto-line, best-approximation, gram-schmidt-2d-then-3d, inner-product-geometry, orthogonal-complement, least-squares-normal-equations — 6), ch.4 (euler-step, euler-vs-rk4, second-order-to-system, diagonalisation-eigenaxes, phase-portrait-2x2, separable-variables-circles, newtons-law-of-cooling, characteristic-roots-regimes — 8), ch.5 (gradient-and-level-curves, directional-derivative, tangent-plane-linearisation, multivariable-limit-paths, chain-rule-on-path, **partial-derivative-as-slice**, **clairaut-mixed-partials** — added tonight — 7), ch.6 (global-extrema-triangle, hesse-eigenvalues, hessian-test, lagrange-multipliers, critical-points-gradient-field, **quadratic-taylor-approximation** — added tonight — 6). Most under-served now: ch.6 (6). Strongest follow-ups: composition-of-matrices (ch.2 — chained linear maps), Cauchy-Schwarz-inequality (ch.3), exact-ODE-recognition or integrating-factor-linear-ODE (ch.4 — would fill the last classification slot), polar-double-integral (ch.5 / new ch.7?), divergence-as-flux (vector calc — may need new chapter row), KKT-or-boundary-extrema (ch.6 — constrained on a closed region), implicit-function-theorem (ch.5 — derive dy/dx along a level curve from ∇f), absolute-extrema-via-boundary-parametrisation (ch.6 — extends global-extrema-triangle to a curved boundary).
