...
- **Next algdat batch — seventh scene after tonight's 2026-06-21 `aktivitetsutvelgelse` (ch.7).**
  Coverage so far: ch.1 `asymptotisk-vekst`, ch.3 `flettesortering`,
  ch.4 `tellesortering-bokser`, ch.5 `binaerhaug-sift`, ch.6
  `dp-fylling-fib`, ch.7 `aktivitetsutvelgelse` (tonight — eight
  activities draw onto a day timeline in original order, slide
  vertically into sorted-by-end order, then a teal sweep pointer
  descends the sorted rows while a violet lastEnd marker hops along
  the timeline; four of eight selected, payoff orden n log n).
  Strongest follow-ups in priority order: `bfs-frontier` (ch.8 Grafer
  ...
  source), `prim-safe-edge` (ch.9 Minimale spenntrær — Prim adds one
  safe edge at a time, growing tree highlighted), `huffman-greedy-merge`
  (ch.7 second take — repeatedly merging the two lowest-weight nodes
  into a code tree if a second greedy scene is wanted before moving
  on to graphs). Skip ch.13 / ch.14 (NP-classes) — they animate poorly.
  All algdat scenes are Norwegian (`language: "no"`); generate-audio.js
...
- **Next ITGK batch — continuing after the 2026-06-20 ch.3 f-strenger-hullene-fylles scene.**
  Eleven ITGK scenes are now in the manifest (founding five, plus
  ...
  `unntak-stiger-oppover` for ch.10 on 2026-06-19, and tonight's
  `f-strenger-hullene-fylles` for ch.3 — two teal value chips glide
  from VarBoxes navn="Ada" and alder=17 into the highlighted slots
  of the template `f"Hei {navn}! Du er {alder} år."`, slots flip from
  amber-italic placeholder to teal mono answer as each chip lands,
  closing beat shows an expression slot `{pris * antall}` evaluating
  through three steps to 100 with the Totalt: 100 kroner output
  band). Strongest next topics, in priority order: for-løkke + range
  (ch.5 — companion to `while-lokke-trace`, index marches through
  the range, accumulator updates each pass; ch.5 already has 1 scene
  so this is depth-not-coverage), variabler / datatyper /
  referanse-vs-verdi (ch.2 — a reference-vs-value model; ch.2 still
  at 0), NumPy array broadcasting (ch.12 still at 0; a row vector
  replicates across a matrix as it adds), Matplotlib live plot of a
  sampled function (ch.13 still at 0; x sweeps the domain, y traces
  the curve). Avoid ch.1 (introduksjon) and ch.15 (IKT-teori) — both
...
- **[HUMAN] Re-run `npm run publish betingelser-jernbanevekslere klasse-og-objekt dict-oppslag-vs-liste-sok slicing-mellom-tegnene asymptotisk-vekst flettesortering binaerhaug-sift unntak-stiger-oppover dp-fylling-fib f-strenger-hullene-fylles tellesortering-bokser aktivitetsutvelgelse`** once Supabase env vars are available on the nightly sandbox. Reviewer skipped publish for the five scenes merged from PR #56 (2026-06-17 nightly) because `.env` was missing on the sandbox — `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` are needed for `scripts/publish-scene.js` to upsert the `public.scenes` rows. The backlog now also includes `unntak-stiger-oppover` and `dp-fylling-fib` from 2026-06-19, `f-strenger-hullene-fylles` (ch.3 ITGK) and `tellesortering-bokser` (ch.4 algdat) from 2026-06-20, and tonight's `aktivitetsutvelgelse` (ch.7 algdat). The scene HTML is already live on Pages (`https://egeiran.github.io/Manimo-Design-System/motion/itgk/<id>.html` and `.../motion/algdat/<id>.html`); only the kort-forklart row is missing. Reviewer flagged on 2026-06-17, extended 2026-06-18, 2026-06-19, 2026-06-20, 2026-06-21.
...
- **algdat chapter map** (Supabase `public.chapters` for
  ...
  problemer. Coverage after the 2026-06-21 nightly (Norwegian, not yet
  published): ch.1 (`asymptotisk-vekst`), ch.3 (`flettesortering`),
  ch.4 (`tellesortering-bokser`), ch.5 (`binaerhaug-sift`), ch.6
  (`dp-fylling-fib`), ch.7 (`aktivitetsutvelgelse` — added tonight,
  eight activities A–H draw onto a day timeline 08:00–21:00 in
  original order, slide vertically into sorted-by-end order, then a
  teal sweep pointer descends the sorted rows while a violet lastEnd
  marker hops along the timeline; selected count badge ticks 0..4 as
  A, D, E, G are picked and B, C, F, H dim to rose; payoff orden n log
  n for the sort + lineær time for the pick). Strongest motion
  follow-ups (priority order): ch.8 BFS frontier, ch.10 Dijkstra
  relaxation, ch.9 Prim safe-edge growth. Skip ch.13/ch.14 (theory,
...
- **ITGK chapter map** (Supabase `public.chapters` for `subject_id='itgk'`,
  ...
  (TDT4109). Coverage after the 2026-06-20 ch.3 nightly (depth-first
  ...
  true branch lights), ch.5 (`while-lokke-trace`), ch.6
