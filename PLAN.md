...
- **2-to-4 decoder gate diagram — rail labels A, !A, B, !B are tightly packed at the top of the rail bank (~22 px apart) in both aspects.** In `motion/ade/two-to-four-decoder.jsx` `GatesDiagram`, the four vertical rails sit at `railSpacingX = 22` (portrait) / `24` (landscape) and the labels at fontSize 12 sit directly above. Readable but cramped. Suggested fix: widen the rail spacing to ~32 px and shift the gate column right to match, OR offset the inverter-rail labels (!A, !B) vertically so they read in two rows instead of one.
...
- **[HUMAN] Re-run `npm run publish norton-equivalent non-inverting-op-amp shift-register ohms-law zener-clipper two-to-four-decoder`** once Supabase env vars are available on the nightly sandbox. Tonight's run added three new ADE scenes that the reviewer should publish after merge. Reviewer flagged on 2026-05-15 (extended 2026-05-17).
...
- **ADE chapter map** (Supabase `public.chapters` for `subject_id='ade'`, 10 chapters as of 2026-05-13): ch.1 Kretsteori, ch.2 Energi og effekt, ch.3 Superposisjon og Thévenin, ch.4 Dioder, ch.5 Digital elektronikk, ch.6 Transistorer, ch.7 Minne og register, ch.8 Reaktive elementer, ch.9 Operasjonsforsterker, ch.10 Digital design. Current scene coverage (after the 2026-05-17 PM run): ch.1 (KVL, voltage-divider, KCL, **ohms-law** — added 2026-05-17 PM), ch.2 (capacitor-energy, inductor-energy, max-power-transfer, joule-heating), ch.3 (Thévenin, Norton, superposition), ch.4 (half-wave-rectifier, full-wave-bridge-rectifier, **zener-clipper** — added 2026-05-17 PM), ch.5 (NAND universality, two's-complement), ch.6 (mosfet-switch, bjt-load-line), ch.7 (D flip-flop, shift-register, SR latch), ch.8 (phasor-rotation, low-pass-bode), ch.9 (inverting op-amp, non-inverting op-amp, summing-amplifier), ch.10 (K-map, FSM, multiplexer-4-to-1, **2-to-4 decoder** — added 2026-05-17 PM). Most under-served chapters now: ch.5 (2 scenes — candidates: hexadecimal / number-systems primer), ch.6 (2 scenes — candidates: CMOS inverter, BJT current-mirror), ch.8 (2 scenes — candidates: RC charging, RLC resonance). Other strong follow-ups: full adder / ripple carry (ch.10 — pending PR #20 resolution), JK flip-flop (ch.7 — deepens memory), integrator op-amp (ch.9 — deepens op-amps), series/parallel (ch.1 — pairs with ohms-law), photodiode / Schottky (ch.4 — pairs with rectifier + zener).
