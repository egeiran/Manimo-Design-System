...
- **integrator-op-amp portrait configuration — diagram parked top-of-canvas leaves big empty band below.** In `motion/ade/integrator-op-amp.jsx` `ConfigurationBeat`, the portrait branch uses `vbW=600, vbH=580` and parks `top: 52%`, so the circuit ends at roughly canvas y=750 and the lower ~530 px sit empty. Either raise the diagram to `top: 44%` and add a labelled note panel below, or grow `vbH` to ~780 and push the caption further down. Reviewer flagged on 2026-05-18.

- **jk-flip-flop portrait symbol — IEEE block parked in upper third with two empty bands.** In `motion/ade/jk-flip-flop.jsx` `SymbolBeat`, the portrait branch has `vbW=600, vbH=500` and the box itself is `260 px` tall starting at y=120. Box ends around canvas y=620 with ~620 px of empty space below. Grow `boxH` to ~340 and `vbH` to ~620 so the symbol breathes into more of the canvas. Reviewer flagged on 2026-05-18.
...
- **[HUMAN] Re-run `npm run publish norton-equivalent non-inverting-op-amp shift-register ohms-law zener-clipper two-to-four-decoder rc-charging cmos-inverter hexadecimal-counting rl-transient integrator-op-amp jk-flip-flop`** once Supabase env vars are available on the nightly sandbox. Tonight's 2026-05-18 run added three more ADE scenes (RL transient ch.8, integrator op-amp ch.9, JK flip-flop ch.7) that the reviewer should publish after merge. Reviewer flagged on 2026-05-15 (extended 2026-05-17 PM, 2026-05-18 PM).
...
- **[HUMAN] Summing-amplifier `CurrentsBeat` landscape — `i_k = V_k / R_k` formula collides with the `V₂` input label.** In `motion/ade/summing-amplifier.jsx` `CurrentsBeat`, the landscape branch parks the formula at `(rInLeftX - 20, rInY[1])` with `textAnchor="end"`, ending the text right where the `V₂` label sits at `(rInLeftX - 14, rInY[1] + 6)`. Reads, but they share the same row. Either move the formula above the top branch (`y = rInY[0] - 28`) or push it left into a dedicated column. Reviewer flagged on 2026-05-17.

- **[HUMAN] Summing-amplifier `FormulaBeat` portrait — main `V_out` formula wraps after the opening paren.** In `motion/ade/summing-amplifier.jsx` `FormulaBeat`, the portrait branch caps the formula at `maxWidth: '20ch'`, so `V_out = -R_f · (V_1/R_1 + V_2/R_2 + V_3/R_3)` wraps to two lines breaking after `(V_1/R_1 +`. Readable, but a manual line break before the parenthesis (or a smaller font like 22 px) would breathe better. Reviewer flagged on 2026-05-17.

- **ADE chapter map** (Supabase `public.chapters` for `subject_id='ade'`, 10 chapters as of 2026-05-13): ch.1 Kretsteori, ch.2 Energi og effekt, ch.3 Superposisjon og Thévenin, ch.4 Dioder, ch.5 Digital elektronikk, ch.6 Transistorer, ch.7 Minne og register, ch.8 Reaktive elementer, ch.9 Operasjonsforsterker, ch.10 Digital design. Current scene coverage (after the 2026-05-18 run): ch.1 (KVL, voltage-divider, KCL, ohms-law), ch.2 (capacitor-energy, inductor-energy, max-power-transfer, joule-heating), ch.3 (Thévenin, Norton, superposition), ch.4 (half-wave-rectifier, full-wave-bridge-rectifier, zener-clipper), ch.5 (NAND universality, two's-complement, hexadecimal-counting), ch.6 (mosfet-switch, bjt-load-line, cmos-inverter), ch.7 (D flip-flop, shift-register, SR latch, **jk-flip-flop** — added tonight), ch.8 (phasor-rotation, low-pass-bode, rc-charging, **rl-transient** — added tonight), ch.9 (inverting op-amp, non-inverting op-amp, summing-amplifier, **integrator-op-amp** — added tonight), ch.10 (K-map, FSM, multiplexer-4-to-1, 2-to-4 decoder). Most under-served chapters now: ch.3 (3 scenes — candidates: mesh / nodal analysis, source transformations), ch.4 (3 scenes — candidates: diode I-V curve, photodiode / Schottky, voltage doubler), ch.5 (3 scenes — candidates: ASCII / character codes, binary addition with carry), ch.6 (3 scenes — candidates: BJT current-mirror, CMOS NAND/NOR). Other strong follow-ups: full adder / ripple carry (ch.10 — pending PR #20 resolution), T flip-flop (ch.7), AC impedance triangle (ch.8), RLC resonance (ch.8).
