...
- **Mat2B chapter map** (Supabase `public.chapters` for `subject_id='mat2b'`, 6 chapters): ch.1 Vektorrom, ch.2 Lineærtransformasjoner, ch.3 Indreproduktrom, ch.4 Differensialligninger, ch.5 Funksjoner og derivasjon, ch.6 Ekstremalpunkter. Coverage after 2026-05-22 nightly: ch.1 (span-and-dependence, dimension-intuition, subspace-test, linear-combination-recipe, **basis-uniqueness**, **null-space-as-line**, **polynomial-vectors** — added tonight — 7), ch.2 (rank-nullity-visual, change-of-basis-matrix, basis-change-grid, matrix-as-function, linear-transformation-grid, determinant-as-area — 6), ch.3 (projection-onto-line, best-approximation, gram-schmidt-2d-then-3d, inner-product-geometry, orthogonal-complement, least-squares-normal-equations — 6), ch.4 (euler-step, euler-vs-rk4, second-order-to-system, diagonalisation-eigenaxes, phase-portrait-2x2 — 5), ch.5 (gradient-and-level-curves, directional-derivative, tangent-plane-linearisation, multivariable-limit-paths, chain-rule-on-path — 5), ch.6 (global-extrema-triangle, hesse-eigenvalues, hessian-test, lagrange-multipliers, critical-points-gradient-field — 5). Most under-served now: ch.4 / ch.5 / ch.6 (5 each). Strongest follow-ups: composition-of-matrices (ch.2 — chained linear maps), Cauchy-Schwarz-inequality (ch.3), exact-vs-separable ODE (ch.4 — first-order classification), polar-double-integral (ch.5 / new ch.7?), divergence-as-flux (vector calc — may need new chapter row), quadratic-taylor-approximation (ch.6 — links linearisation → Hessian test), KKT-or-boundary-extrema (ch.6 — constrained on a closed region).
