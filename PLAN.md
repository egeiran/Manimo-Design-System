...
- **Next ALGDAT batch — second wave after the 2026-06-14 founding scene.**
  The first algdat scene (`flettesortering`, ch.3 Splitt og hersk) is in
  flight tonight as PR `nightly/2026-06-14-algdat`. All 15 algdat chapters
  in Supabase are still 0–1 scenes covered. Strongest next picks in
  priority order: `asymptotisk-vekstrace` (ch.1 — growth curves O(1) /
  O(log n) / O(n) / O(n log n) / O(n²) racing apart as n grows, dominating
  term taking over), `heap-sift-down` (ch.5 — a heap node sinking through
  its children until the heap property is restored), `dp-fib-table`
  (ch.6 — a Fibonacci or coin-change DP table filling cell-by-cell,
  each cell pulling from its predecessors), `bfs-frontier` (ch.8 — visited
  frontier expanding from a source across a small graph, queue updating
  with each step), `counting-sort-buckets` (ch.4 — elements falling into
  count buckets, then concatenating back into a sorted row). Avoid ch.13/14
  (NP/complexity-class theory — animates poorly). All algdat scenes are
  Norwegian (`language: "no"`); generate-audio.js auto-pins voice Liam
  `TX3LPaxmHKxFdv7VOQHJ` + `eleven_turbo_v2_5` + `language_code: "no"`.
  Match the ITGK/algdat house pattern: CodeBlock with
  `fontVariantLigatures: 'none'`, milestones as fractions of sprite
  duration, cell/pointer/frame vocabulary.
...
- **ALGDAT chapter map** (Supabase `public.chapters` for `subject_id='algdat'`,
  15 chapters, TDT4120): ch.0 Verktøykassa: notasjon og forkunnskaper,
  ch.1 Algoritmer, kjøretid og asymptotisk notasjon, ch.2 Problemer,
  instanser og reduksjoner, ch.3 Splitt og hersk, ch.4 Sortering i lineær
  tid og utvalg, ch.5 Hauger og binære søketrær, ch.6 Dynamisk
  programmering, ch.7 Grådige algoritmer, ch.8 Grafer og traversering,
  ch.9 Minimale spenntrær, ch.10 Korteste vei fra én kilde, ch.11
  Korteste vei mellom alle par, ch.12 Maksimal flyt, ch.13
  Kompleksitetsklasser og NP-kompletthet, ch.14 NP-komplette problemer og
  bevis. Coverage after the 2026-06-14 founding scene (Norwegian, not yet
  published — awaiting reviewer merge): ch.3 (`flettesortering` — merge
  sort with recursion tree + two-pointer merge motion). All other
  chapters 0 scenes. **Norwegian audio pin:** voice Liam
  `TX3LPaxmHKxFdv7VOQHJ`, model `eleven_turbo_v2_5`, `language_code:
  "no"` — wired as automatic default in generate-audio.js for
  `language: "no"`. Theory-heavy ch.13/14 animate poorly — skip until a
  strong visual angle appears. Strongest immediate follow-ups live in the
  [AGENT] queue item above.
