...
- **Next ITGK batch — continuing after the 2026-06-27 ch.14 arv-og-overstyring scene.**
  ...
  exits ALL enclosing loops); shallow-vs-deep-copy (ch.8 —
  `import copy`; a nested list with `b = a`, `b = a.copy()`,
  `b = copy.deepcopy(a)` side by side, only deepcopy unhooks the
  inner list when mutated); valgsortering-finn-minste (ch.11 —
...
- **[HUMAN] Re-run `npm run publish ford-fulkerson-augment`** (plus the rest of tonight's PR #78 backlog: `aktivitetsvalg-graadig asymptotisk-vekst bfs-bolgen binaerhaug-sift dijkstra-slapp-av dp-fylling-fib flettesortering floyd-warshall-mellomstopp kruskal-trygg-kant tellesortering-bokser arv-og-overstyring betingelser-jernbanevekslere dict-oppslag-vs-liste-sok f-strenger-hullene-fylles for-lokke-range innstikksortering-kortene klasse-og-objekt matplotlib-funksjonskurve nestede-lokker numpy-broadcasting referanse-vs-verdi slicing-mellom-tegnene unntak-stiger-oppover`) once Supabase env vars are available on the nightly sandbox. The 2026-06-28 reviewer skipped publish for all 24 scenes merged from PR #78 because `.env` was missing on the sandbox — `SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` are needed for `scripts/publish-scene.js` to upsert the `public.scenes` rows. Scene HTML is already live on Pages (`https://egeiran.github.io/Manimo-Design-System/motion/algdat/<id>.html` and `.../motion/itgk/<id>.html`); only the kort-forklart rows are missing. Reviewer flagged on 2026-06-28.
