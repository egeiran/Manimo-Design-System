...
- **rc-charging portrait timeConstant — graph occupies less than half the vertical canvas.** In `motion/ade/rc-charging.jsx` `TimeConstantBeat`, the portrait branch parks a 600×500 viewBox at top:50% so the SVG (sized to `G.vbH - 110 = 390 px`) leaves ~440 px of empty space above and below. Either centre the column more tightly with a smaller `gap` and a taller graph, or push the curve markers (τ at 63%, 5τ at 99%) larger so the graph fills the canvas. Reviewer flagged on 2026-05-17.
...
- **[HUMAN] Re-run `npm run publish norton-equivalent non-inverting-op-amp shift-register ohms-law zener-clipper two-to-four-decoder rc-charging cmos-inverter hexadecimal-counting`** once Supabase env vars are available on the nightly sandbox. Tonight's run added three new ADE scenes (RC charging ch.8, CMOS inverter ch.6, hexadecimal counting ch.5) that the reviewer should publish after merge. Reviewer flagged on 2026-05-15 (extended 2026-05-17 PM).
...
- **ADE chapter map** (Supabase `public.chapters` for `subject_id='ade'`, 10 chapters as of 2026-05-13): ch.1 Kretsteori, ch.2 Energi og effekt, ch.3 Superposisjon og Thévenin, ch.4 Dioder, ch.5 Digital elektronikk, ch.6 Transistorer, ch.7 Minne og register, ch.8 Reaktive elementer, ch.9 Operasjonsforsterker, ch.10 Digital design. Current scene coverage (after the second 2026-05-17 PM run): ch.1 (KVL, voltage-divider, KCL, ohms-law), ch.2 (capacitor-energy, inductor-energy, max-power-transfer, joule-heating), ch.3 (Thévenin, Norton, superposition), ch.4 (half-wave-rectifier, full-wave-bridge-rectifier, zener-clipper), ch.5 (NAND universality, two's-complement, **hexadecimal-counting** — added tonight), ch.6 (mosfet-switch, bjt-load-line, **cmos-inverter** — added tonight), ch.7 (D flip-flop, shift-register, SR latch), ch.8 (phasor-rotation, low-pass-bode, **rc-charging** — added tonight), ch.9 (inverting op-amp, non-inverting op-amp, summing-amplifier), ch.10 (K-map, FSM, multiplexer-4-to-1, 2-to-4 decoder). Most under-served chapters now: ch.5 (3 scenes — candidates: ASCII / character codes, binary addition with carry), ch.6 (3 scenes — candidates: BJT current-mirror, CMOS NAND/NOR), ch.7 (3 scenes — candidates: JK flip-flop, T flip-flop, ring counter), ch.8 (3 scenes — candidates: RL transient, RLC resonance, AC impedance triangle). Other strong follow-ups: full adder / ripple carry (ch.10 — pending PR #20 resolution), integrator op-amp (ch.9), series/parallel (ch.1), photodiode / Schottky (ch.4).
