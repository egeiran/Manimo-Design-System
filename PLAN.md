...
- **[HUMAN] Re-run `npm run publish wheatstone-bridge op-amp-comparator demultiplexer-1-to-4`** once Supabase env vars are available on the nightly sandbox. Tonight's reviewer skipped publish because keys were missing (`.env` absent in the sandbox after fresh clone); PR #34 merged green, the three new scenes are in `motion/scene-manifest.json` with correct `subject_id=ade` + `chapter_number` (3/9/10) but no `public.scenes` row yet. Reviewer flagged on 2026-05-20.

- **ADE chapter map** (Supabase `public.chapters` for `subject_id='ade'`, 10 chapters as of 2026-05-13): ch.1 Kretsteori, ch.2 Energi og effekt, ch.3 Superposisjon og Thévenin, ch.4 Dioder, ch.5 Digital elektronikk, ch.6 Transistorer, ch.7 Minne og register, ch.8 Reaktive elementer, ch.9 Operasjonsforsterker, ch.10 Digital design. Current scene coverage (after the 2026-05-20 v2 run): ch.1 (KVL, voltage-divider, KCL, ohms-law, current-divider — 5), ch.2 (capacitor-energy, inductor-energy, max-power-transfer, joule-heating — 4), ch.3 (Thévenin, Norton, superposition, source-transformations, **wheatstone-bridge** — added tonight — 5), ch.4 (half-wave-rectifier, full-wave-bridge-rectifier, zener-clipper, diode-iv-curve — 4), ch.5 (NAND universality, two's-complement, hexadecimal-counting, binary-addition, full-adder — 5), ch.6 (mosfet-switch, bjt-load-line, cmos-inverter, bjt-current-mirror — 4), ch.7 (D flip-flop, shift-register, SR latch, jk-flip-flop, ripple-counter — 5), ch.8 (phasor-rotation, low-pass-bode, rc-charging, rl-transient, rlc-resonance, high-pass-bode — 6), ch.9 (inverting op-amp, non-inverting op-amp, summing-amplifier, integrator-op-amp, **op-amp-comparator** — added tonight — 5), ch.10 (K-map, FSM, multiplexer-4-to-1, 2-to-4 decoder, **demultiplexer-1-to-4** — added tonight — 5). Most under-served chapters now: ch.2 (4 — power factor, AC average power, impedance/power triangle still open), ch.4 (4 — diode clipper/clamper, voltage doubler, LED biasing), ch.6 (4 — emitter follower, MOSFET transfer characteristic). Other strong follow-ups: T flip-flop standalone (ch.7), AC impedance triangle (ch.8), 8-to-3 priority encoder (ch.10), mesh/nodal analysis (ch.1, ch.3), Schmitt trigger (ch.9 — companion to comparator).
