...
- **Full-wave bridge rectifier `halfCycles` beat — `BALANCE` truth-table look for portrait label spacing.** In `motion/ade/full-wave-bridge-rectifier.jsx` `HalfCyclesBeat`, the "I↓" load arrow sits to the LEFT of the load resistor at `loadX - 28`; in portrait this puts it very close to the bridge `E` (AC2) label. Either move the load arrow to the RIGHT side of the resistor in portrait, or shrink the `I↓` text to fontSize 10.
...
- **[HUMAN] PR #20's `full-adder` scene also conflicts with the next-night candidate.** PR #20 (still open) introduces a `full-adder` scene at `chapter_number: 11`; PLAN's current chapter map points full-adder to ch.10 (Digital design). Tonight's nightly author picked `multiplexer-4-to-1` (ch.10) instead to dodge the collision, but next-night runs should still pick up full-adder/ripple-carry once PR #20 is resolved — confirm whether PR #20's variant ships under ch.10 (re-mapped) before authoring a competing version.
...
- **ADE chapter map** (Supabase `public.chapters` for `subject_id='ade'`, 10 chapters as of 2026-05-13): ch.1 Kretsteori, ch.2 Energi og effekt, ch.3 Superposisjon og Thévenin, ch.4 Dioder, ch.5 Digital elektronikk, ch.6 Transistorer, ch.7 Minne og register, ch.8 Reaktive elementer, ch.9 Operasjonsforsterker, ch.10 Digital design. Current scene coverage: ch.1 (KVL, voltage-divider, KCL — added 2026-05-15), ch.2 (capacitor-energy), ch.3 (Thévenin), ch.4 (half-wave-rectifier, full-wave-bridge-rectifier — added 2026-05-15), ch.5 (NAND universality, two's-complement), ch.6 (mosfet-switch, bjt-load-line), ch.7 (D flip-flop), ch.8 (phasor-rotation, low-pass-bode), ch.9 (inverting op-amp), ch.10 (K-map, FSM, multiplexer-4-to-1 — added 2026-05-15). All 10 chapters now have ≥ 1 scene. Strongest under-served candidates for upcoming runs: full adder / ripple carry (ch.10 — pending PR #20 resolution), MOSFET inverter cascade / CMOS (ch.6 deepening), RC charging in the ADE namespace (ch.8 reactive), Norton equivalent (ch.3 — pair with Thévenin), binary decoder 2-to-4 (ch.10 — pair with MUX).
