...
- **[HUMAN] Re-run `npm run publish all`** once Supabase env vars are wired into the nightly sandbox. Two cumulative backlogs are pending: (a) the 2026-05-13 chapter-remap that moved 8 of 9 ADE scenes to their corrected `chapter_number` values (specs + manifest updated locally, not yet pushed to Supabase) plus the earlier `basis-change-grid` ch.1→ch.2 fix that reconciles the local/remote mismatch flagged by `npm run coverage mat2b`; (b) every nightly-added scene that has not been published yet — including `norton-equivalent non-inverting-op-amp shift-register ohms-law zener-clipper two-to-four-decoder rc-charging cmos-inverter hexadecimal-counting rl-transient integrator-op-amp jk-flip-flop source-transformations rlc-resonance binary-addition diode-iv-curve bjt-current-mirror full-adder current-divider high-pass-bode ripple-counter wheatstone-bridge op-amp-comparator demultiplexer-1-to-4 diode-clamper power-triangle priority-encoder emitter-follower schmitt-trigger t-flip-flop mosfet-transfer-characteristic voltage-doubler ac-impedance-triangle` plus Mat2B trio `subspace-test linear-combination-recipe determinant-as-area` and tonight's PM2 ADE trio `mesh-analysis delta-y-conversion bcd-seven-segment-decoder`. The new entries are in `motion/scene-manifest.json` with correct `subject_id` + `chapter_number` but no `public.scenes` row yet. Originally flagged 2026-05-15, extended through 2026-05-21 PM2.

- **ADE chapter map** (Supabase `public.chapters` for `subject_id='ade'`, 10 chapters as of 2026-05-13): ch.1 Kretsteori, ch.2 Energi og effekt, ch.3 Superposisjon og Thévenin, ch.4 Dioder, ch.5 Digital elektronikk, ch.6 Transistorer, ch.7 Minne og register, ch.8 Reaktive elementer, ch.9 Operasjonsforsterker, ch.10 Digital design. Current scene coverage (after the 2026-05-21 PM2 run): ch.1 (KVL, voltage-divider, KCL, ohms-law, current-divider, **mesh-analysis** — added tonight — 6), ch.2 (capacitor-energy, inductor-energy, max-power-transfer, joule-heating, power-triangle — 5), ch.3 (Thévenin, Norton, superposition, source-transformations, wheatstone-bridge, **delta-y-conversion** — added tonight — 6), ch.4 (half-wave-rectifier, full-wave-bridge-rectifier, zener-clipper, diode-iv-curve, diode-clamper, voltage-doubler — 6), ch.5 (NAND universality, two's-complement, hexadecimal-counting, binary-addition, full-adder, **bcd-seven-segment-decoder** — added tonight — 6), ch.6 (mosfet-switch, bjt-load-line, cmos-inverter, bjt-current-mirror, emitter-follower, mosfet-transfer-characteristic — 6), ch.7 (D flip-flop, shift-register, SR latch, jk-flip-flop, ripple-counter, t-flip-flop — 6), ch.8 (phasor-rotation, low-pass-bode, rc-charging, rl-transient, rlc-resonance, high-pass-bode, ac-impedance-triangle — 7), ch.9 (inverting op-amp, non-inverting op-amp, summing-amplifier, integrator-op-amp, op-amp-comparator, schmitt-trigger — 6), ch.10 (K-map, FSM, multiplexer-4-to-1, 2-to-4 decoder, demultiplexer-1-to-4, priority-encoder — 6). Most under-served now: ch.2 Energi og effekt (5). Strongest follow-ups for ch.2: AC power-factor correction (apparent-vs-real-power distinct from the existing power-triangle), instantaneous-vs-average-power on a resistive load with sinusoidal drive, RMS as the equivalent DC value, three-phase power balance. Other gaps: nodal analysis (ch.1 — sibling to tonight's mesh-analysis), Millman's theorem (ch.3 — collapses parallel branches with V sources, sibling to tonight's delta-Y), Schottky diode I-V or LED forward-voltage chart (ch.4), JFET as alternative transistor topic (ch.6), 555-timer architecture (ch.10).

- **Mat2B chapter map** (Supabase `public.chapters` for `subject_id='mat2b'`, 6 chapters): ch.1 Vektorrom, ch.2 Lineærtransformasjoner, ch.3 Indreproduktrom, ch.4 Differensialligninger, ch.5 Funksjoner og derivasjon, ch.6 Ekstremalpunkter. Coverage after 2026-05-20 nightly: ch.1 (span-and-dependence, dimension-intuition, **subspace-test**, **linear-combination-recipe** — 4), ch.2 (rank-nullity-visual, change-of-basis-matrix, basis-change-grid, matrix-as-function, linear-transformation-grid, **determinant-as-area** — 6), ch.3 (projection-onto-line, best-approximation, gram-schmidt-2d-then-3d — 3), ch.4 (5), ch.5 (4), ch.6 (3). Most under-served now: ch.3 Indreproduktrom (3 — orthogonal-complement, inner-product-geometry, least-squares-normal-equations still open) and ch.6 Ekstremalpunkter (3 — Lagrange multipliers is the biggest missing topic). Other strong follow-ups: basis-uniqueness (ch.1 — every vector has unique coordinates in a basis), composition-of-matrices (ch.2), chain-rule-on-path (ch.5), polar-double-integral (ch.5 / new ch.7?), divergence-as-flux (vector calc — may need new chapter row).
