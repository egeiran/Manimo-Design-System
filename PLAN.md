...
- **[HUMAN] Re-run `npm run publish basis-uniqueness null-space-as-line polynomial-vectors`** once Supabase env vars are available on the nightly sandbox. Tonight's reviewer skipped publish because keys were missing. (Already included in the cumulative backlog item below, but logged here separately so the 2026-05-22 PR-42 publish step is traceable on its own.)

- **[HUMAN] Re-run `npm run publish separable-variables-circles newtons-law-of-cooling characteristic-roots-regimes`** once Supabase env vars are wired in. Three new ch.4 Differensialligninger scenes added tonight (2026-05-22 PM nightly); manifest + spec carry `subject_id='mat2b'` + `chapter_number=4` but `public.scenes` rows have not been written yet.

- **[HUMAN] Re-run `npm run publish quadratic-taylor-approximation partial-derivative-as-slice clairaut-mixed-partials`** once Supabase env vars are wired in. Three new Mat2B scenes added in the 2026-05-23 morning nightly: `quadratic-taylor-approximation` (ch.6 — links tangent-plane-linearisation → hessian-test via a saddle-slice order sweep with live residual bar), `partial-derivative-as-slice` (ch.5 — geometric meaning of ∂f/∂x with a scrubbing slicing plane y = y₀ and live slope readout), `clairaut-mixed-partials` (ch.5 — symmetry of mixed partials via two-path computation + shrinking corner-sum rectangle around (0.9, 0.5)). Manifest + spec carry `subject_id='mat2b'` with chapters 6, 5, 5 respectively but `public.scenes` rows have not been written yet.

- **[HUMAN] Re-run `npm run publish gaussian-elimination-2d matrix-product-as-composition rotation-matrix-family`** once Supabase env vars are wired in. Three new Mat2B chapter-2 (Lineærtransformasjoner) scenes added in the 2026-05-23 PM nightly: `gaussian-elimination-2d` (geometric row reduction — two lines morph through row ops while the intersection (2, 1) stays pinned, ending in axis-aligned echelon form), `matrix-product-as-composition` (AB = "B first, then A" — apply rotation A to a B-stretched unit square in two steps; swap order to land on a different parallelogram, drives AB ≠ BA home), `rotation-matrix-family` (R(θ) continuously swept from 0 to 2π with basis vectors tracing the unit circle; angle-addition R(30°)·R(45°) = R(75°)). Manifest + spec carry `subject_id='mat2b'` with `chapter_number=2` for all three but `public.scenes` rows have not been written yet.

- **[HUMAN] Re-run `npm run publish all`** once Supabase env vars are wired into the nightly sandbox. Two cumulative backlogs are pending: (a) the 2026-05-13 chapter-remap that moved 8 of 9 ADE scenes to their corrected `chapter_number` values (specs + manifest updated locally, not yet pushed to Supabase) plus the earlier `basis-change-grid` ch.1→ch.2 fix that reconciles the local/remote mismatch flagged by `npm run coverage mat2b`; (b) every nightly-added scene that has not been published yet — including `norton-equivalent non-inverting-op-amp shift-register ohms-law zener-clipper two-to-four-decoder rc-charging cmos-inverter hexadecimal-counting rl-transient integrator-op-amp jk-flip-flop source-transformations rlc-resonance binary-addition diode-iv-curve bjt-current-mirror full-adder current-divider high-pass-bode ripple-counter wheatstone-bridge op-amp-comparator demultiplexer-1-to-4 diode-clamper power-triangle priority-encoder emitter-follower schmitt-trigger t-flip-flop mosfet-transfer-characteristic voltage-doubler ac-impedance-triangle` plus Mat2B `subspace-test linear-combination-recipe determinant-as-area inner-product-geometry orthogonal-complement least-squares-normal-equations lagrange-multipliers chain-rule-on-path critical-points-gradient-field basis-uniqueness null-space-as-line polynomial-vectors`. The new entries are in `motion/scene-manifest.json` with correct `subject_id` + `chapter_number` but no `public.scenes` row yet. Originally flagged 2026-05-15, extended through 2026-05-22.
...
- **Mat2B chapter map** (Supabase `public.chapters` for `subject_id='mat2b'`, 6 chapters): ch.1 Vektorrom, ch.2 Lineærtransformasjoner, ch.3 Indreproduktrom, ch.4 Differensialligninger, ch.5 Funksjoner og derivasjon, ch.6 Ekstremalpunkter. Coverage after 2026-05-23 PM nightly: ch.1 (span-and-dependence, dimension-intuition, subspace-test, linear-combination-recipe, basis-uniqueness, null-space-as-line, polynomial-vectors — 7), ch.2 (rank-nullity-visual, change-of-basis-matrix, basis-change-grid, matrix-as-function, linear-transformation-grid, determinant-as-area, **gaussian-elimination-2d**, **matrix-product-as-composition**, **rotation-matrix-family** — added tonight — 9), ch.3 (projection-onto-line, best-approximation, gram-schmidt-2d-then-3d, inner-product-geometry, orthogonal-complement, least-squares-normal-equations — 6), ch.4 (euler-step, euler-vs-rk4, second-order-to-system, diagonalisation-eigenaxes, phase-portrait-2x2, separable-variables-circles, newtons-law-of-cooling, characteristic-roots-regimes — 8), ch.5 (gradient-and-level-curves, directional-derivative, tangent-plane-linearisation, multivariable-limit-paths, chain-rule-on-path, partial-derivative-as-slice, clairaut-mixed-partials — 7), ch.6 (global-extrema-triangle, hesse-eigenvalues, hessian-test, lagrange-multipliers, critical-points-gradient-field, quadratic-taylor-approximation — 6). Most under-served now: ch.6 (6), then ch.3 (6). Strongest follow-ups: Cauchy-Schwarz-inequality (ch.3), QR-factorisation-via-Gram-Schmidt (ch.3 — extends the existing gram-schmidt scene), exact-ODE-recognition or integrating-factor-linear-ODE (ch.4 — would fill the last classification slot), polar-double-integral (ch.5 / new ch.7?), divergence-as-flux (vector calc — may need new chapter row), KKT-or-boundary-extrema (ch.6 — constrained on a closed region), implicit-function-theorem (ch.5 — derive dy/dx along a level curve from ∇f), absolute-extrema-via-boundary-parametrisation (ch.6 — extends global-extrema-triangle to a curved boundary), shear-decomposition (ch.2 — every 2×2 matrix factors as rotation × scaling × rotation via SVD).
